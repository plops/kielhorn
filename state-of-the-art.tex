...
Fluorescence of molecules is a field of science on its own. Books on
molecular physics like \cite{2006Haken} go much more into detail. This
section just gives a basic overview of the theory.

Lets think of a really simple molecule. We take two hydrogen atoms,
grab their nucleii and move them slowly together. When the nucleii
have a big distance the atoms exist as two separate entities without
...
\begin{figure}[!hbt]
  \centering
  \includegraphics[width=7cm]{flu-potential}
  \caption{Electron density maps and potential curves for ground
    $\H_2$ and excited states $\H_2^*$ of dihydrogen and the
    dihydrogen ion $\H_2^+$ (from \cite{2006Haken} p.~258).}
  \label{fig:flu-potential}
\end{figure}
When the nucleii have an intermediate distance between zero and
infinity one can express molecular orbitals as a linear combination of
atomic orbitals and calculate their potential energy. The result of
this calculation is shown in \figref{fig:flu-potential}. The graph
...
In the same way one can combine a ground state hydrogen atom $\H$ and
...
\begin{fenster}
  ...
  {\small
    ...
    \item[$\Omega$] 
      ...
      Electronic angular momentum $\Omega$ in direction of the nuclear axis.
...
\begin{figure}[!hbt]
  \centering
  % (423-120)/423*7 = 4.77 
  \includegraphics[width=4.77cm,trim=0 0 135 0,clip]{flu-condon}
...
state. So the minima of both curves are exactly on top of each
...
\begin{figure}[!hbt]
  \centering
  \includegraphics[width=7cm]{flu-level}
...
to the Franck-Condon principle the excited state also consists of
...
with electrons of other fluorescent molecules\footnote{This is a good
  thing for most microscopic applications, as images of incoherent
  light sources resemble the object space better than coherent
  sources.}. A $S_2$ electron will decay radiationless into $S_1$ due
...
The energy of the emitted fluorescence photon is generally lower than
the energy of the excitation photon. The fluorescence lifetime -- the
...
(in ethanol $\eta=0.94$ \cite{1996Fischer}) or antracene
(9,10-diphenyl anthracene in cyclohexane $\eta=0.90$ \cite{1983Hamai})
...
fluorescence signal -- the \emph{photobleaching}.  Another effect is
that the excited fluorophores impair living cells.  It is a curious
quirk of nature that the ground state of the oxygen molecule is a
triplet state ${}^3\O_2$.  If an oxygen molecule hits a $T_1$
fluorophore, e.g.\ ${}^3\textsf{Chlorophyll}^*$, the energy of the dye
can be transferred to the oxygen (\cite{2006Haken} p.438):
...
inside living creatures. Plants have a concurring mechanism that is
enabled if there is too much light. They transfer the energy of the
chlorophyll onto carotenoid molecules and can prevent the hazardous
\emph{phototoxicity} effects of singlet oxygen.
...
solvent (\cite{2008Vogelsang}). For fixed samples it helps to use
