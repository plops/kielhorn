...
$S1$ (see \figref{fig:flu-condon}, right).
...
(see \figref{fig:oxygen}).  If an oxygen molecule comes into physical
...
chloroplasts in order to expose them to less light. However, they even
have a molecular protection mechanism: They transfer the energy of the
...
use replace water as the solvent, e.g.\ with a polymer.
...
behind the tubelens TL1 \citep{Botcherby2007,Botcherby2008a}.
...
\begin{figure}[!hbt]
  ...
  \caption{Schematic to depicting $k_xk_z-$cross sections of the
    support of optical transfer function for microscope objectives
    with different collection angles. {\bf left \& middle:}
    Objectives, that only collect one light that is directed into one
    half space, have the missing cone problem. There, low spatial
...
lower efficiency (in the excitation path) is hardly an issue. Therefore
