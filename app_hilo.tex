...
\figref{fig:input} left. Note that the sphere is very faint but it is
...
\nomenclature{PSF}{point spread function} \nomenclature{OTF}{optical
  transfer function} of the microobjective (this time according to the
...
\figref{fig:input}. The right picture shows the image for non-uniform
illumination with the \unit[600]{nm}-period grating.
\begin{figure}[htb]
  \centering
  \subfigure[object]{\includegraphics[width=4cm]{../app_hilo/obj}}
  \subfigure[widefield illumination]{\includegraphics[width=4cm]{../app_hilo/iu}}
  \subfigure[structured illumination]{\includegraphics[width=4cm]{../app_hilo/in}}
...
\subsection{Apotome}
Using non-uniform excitation light \cite{1997Neil} circumvents this
...
\subsubsection{Using speckle pattern as non-uniform illumination (speckle method)}
...
\begin{figure}[htb]
  \centering
  \subfigure[High frequency components $I_\textrm{hp}$ of widefield.]{\includegraphics[width=4cm]{../app_hilo/ihp}}
  ...
  \caption{More intermediate and the end result of the HiLo
    ``speckle'' algorithm. The white circle in image {\bf (b)} marks
    the cutoff frequency $k_c$ of the spatial low pass filter. The
    scalebars are \unit[2]{$\mu$m} wide.}
...
\begin{figure}[htb]
  ...
  \subfigure[Fouriertransform $\tilde
...
image is displayed in \figref{fig:ihilo2}.
\begin{figure}[htb]
  \centering
...
\figref{fig:cov3d}. The next listing is the code to locate the center
