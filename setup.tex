...
Of course the sphere model is rather crude and it can not convey the
...
\imagw{12cm}{scan-mosaic_testsample_n100_3_7_5_big_label}{Test case
  for spatio-angular illumination.  {\bf (A-C)} Rays from the back
  focal plane are traced through the target sphere and the total
  intersection length with out-of-focus spheres is plotted in the
  diagrams.  {\bf D)} The target sphere is moved out of a layer of
  spheres. {\bf E)} Rays start from a circular window with radius $r$
...
\imagw{12cm}{scan-mosaic_nuc12_n100_3_7_5_big_label}{Raytrace for
  ...
  nucleus. The table below the yellow BFP maps contains the maximum
  and minimum values of each map.}
