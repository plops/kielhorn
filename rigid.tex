...
\figref{fig:rigid-pics}~left for a uniform widefield image) is imaged
...
%% bright image is at position 102 in the stack
% diphist(a(:,:,102),[min(a),max(a)])

%% bright image has minimum at 800 ADU
bright = squeeze(a(:,:,102));
% create mask with illuminated area
mask = gaussf(bright,8) > 800;

%% the background is be 510 ADU
...
cod = [']$ fit:minpack_lsquares(g(s,p,x,y),[s,p,x,y],[.88,-1.3,1200,-20]);' ...
 'write_data(fit[1],"/home/martin/max.out");']

%% write maxima commands into file and execute
