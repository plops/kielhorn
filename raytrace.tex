...
sphere be centred in $\c$ with radius $R$. Their two equations
\begin{align}
  (\r-\c)^2&=R^2\\
  \r&=\s+\tau\hd
\end{align}
...
\begin{align}
  ...
  R &= \boxed{nf}
\end{align}
The principal surface\footnote{An image forming system focuses
...
the image point (\cite{Smith2000} p.~22 and \cite{Botcherby2008}). In
the paper \citep{Hwang2008} they express the deviation between the
real principal surface and the principal plane with an approximation
...
the target $\c$ and the nucleus centre $\s$. The direction $\y$ must
